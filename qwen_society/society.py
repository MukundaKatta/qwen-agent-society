...
class Society:
    def __init__(self, client: _Client, roster: Optional[dict[str, AgentSpec]] = None, max_rounds: int = 2) -> None:
        self.client = client
    ...
    def run(self, task: str) -> SocietyResult:
        ...
        plan_out = self.client.complete(self.roster.get("planner", PLANNER), f"TASK: {task}")
        ...
        for rnd in range(1, self.max_rounds + 1):
            ...
            crit_content = "PARTIALS:\n" + "\n".join(f"{r}: {c}" for r, c in partials.items())
            crit_out = self.client.complete(self.roster.get("critic", CRITIC), crit_content)
            bb.post(Message("critic", MessageKind.CRITIQUE, "critique", crit_out, rnd))

            if crit_out.strip().upper().startswith("APPROVE"):
                approved = True
                break
            note = crit_out.split(":", 1)[1].strip() if ":" in crit_out else crit_out.strip()
            critic_notes = {st.role: note for st in subtasks}

        partials = bb.latest_partials()
        syn_content = "APPROVED PARTIALS:\n" + "\n".join(f"{r}: {c}" for r, c in partials.items())
        final = self.client.complete(self.roster.get("synthesizer", SYNTHESIZER), syn_content)
        bb.post(Message("synthesizer", MessageKind.FINAL, "final", final, rounds))

        return SocietyResult(task=task, final=final, rounds=rounds, approved=approved, transcript=bb.transcript())
